...
# Date last modified: September 1 2021 by Shireen Assaf 
# *****************************************************************************************************/
# 
# /*----------------------------------------------------------------------------//
...
# //PNC timing for mother	
   IRdata <- IRdata %>%
     mutate(rh_pnc_wm_timing =
              case_when(
                (m51_1 >=242 & m51_1<=297) | (m51_1>=306 & m51_1<397)  ~ 0 ,
                m51_1  %in% c(100, 101, 102, 103)  ~ 1 ,
                m51_1 >=104 & m51_1<=123  ~ 2 ,
                ...
                m51_1  %in% c(198, 199, 298, 299, 298, 399, 998, 999)  ~ 9 ,
                m50_1==0 | m50_1==9 ~ 0,
                (m52_1>29 & m52_1<97)  ~ 0, 
                bidx_01!=1 | age>=24 ~ 99 )) %>%
   ...
   IRdata <- IRdata %>%
     mutate(rh_pnc_wm_pv =
              case_when(
                ...
                m52_1>96 ~ 9 ,
                bidx_01!=1 | age>=24 ~ 99 )) %>%
     replace_with_na(replace = list(rh_pnc_wm_pv = c(99))) %>%
     ...
     set_variable_labels(rh_pnc_wm_pv = "Timing after delivery for mother's PNC check")
}
    ...
    mutate(momcheck =
             case_when(
               (m62_1==1 | m66_1==1) & age<24 ~ 1,
                age<24   ~ 0 )) %>%
    mutate(pnc_wm_time00 = # 	create temp vars for calculations
             case_when(
               momcheck==0 & age<24 ~ 0 , # 	
               m64_1>=11 & m64_1<30 & age<24 ~ as.numeric(m63_1))) %>%
    mutate(pnc_wm_time0 = 
             case_when(
              momcheck==0 & age<24 ~ 0 , # 
              m64_1>=11 & m64_1<30 & age<24 ~ as.numeric(m63_1) ,
              pnc_wm_time00 <1000 & m64_1>30 & m64_1<100 & age<24 ~ 0 )) %>%  
  mutate(pnc_wm_time = 
           case_when(
             momcheck==0 & age<24 ~ 0 , # 	*Add in women who had no check 
             m64_1>=11 & m64_1<30 & age<24 ~ as.numeric(m63_1) , # start with women who delivered in a health facility with a check
             pnc_wm_time00 <1000 & m64_1>30 & m64_1<100 & age<24 ~ 0 , # 	*Account for provider of PNC- country specific- see table footnotes
             pnc_wm_time0==999 & m68_1>=11 & m68_1<30 & age<24 ~ as.numeric(m67_1) , # 	*Add in women who delivered at home with a check
             m67_1<1000 & m68_1>30 & m68_1<100 & age<24 ~ 0  # 	*Account for provider of PNC- country specific- see table footnotes
             ))
...
#Providers of PNC for facility deliveries   
  IRdata <- IRdata %>%
    mutate(pnc_wm_pv_hf =
             case_when(
               rh_pnc_wm_2days==1 & m64_1==0 ~ 0,
               rh_pnc_wm_2days==0 ~ 0
               rh_pnc_wm_2days==1 & m64_1 ==11 ~ 1,
               rh_pnc_wm_2days==1 & m64_1 %in% c(12,13) ~ 2,
               rh_pnc_wm_2days==1 & m64_1 %in% c(14,15) ~ 3,
               rh_pnc_wm_2days==1 & m64_1>=16 & m52_1<=90 ~ 4,
               rh_pnc_wm_2days==1 & m64_1==96 ~ 5,
               rh_pnc_wm_2days==1 & m64_1>96 ~ 9 ,
               bidx_01!=1 | age>=24 ~ 99 )) %>%
...
#Providers of PNC for home deliveries or checks after discharge    
    ...
    mutate(pnc_wm_pv_home =
             case_when(
               rh_pnc_wm_2days==1 & m68_1==0 ~ 0,
               rh_pnc_wm_2days==0 ~ 0
               rh_pnc_wm_2days==1 & m68_1 ==11 ~ 1,
               rh_pnc_wm_2days==1 & m68_1 %in% c(12,13) ~ 2,
               rh_pnc_wm_2days==1 & m68_1 %in% c(14,15) ~ 3,
               rh_pnc_wm_2days==1 & m68_1>=16 & m52_1<=90 ~ 4,
               rh_pnc_wm_2days==1 & m68_1==96 ~ 5,
               rh_pnc_wm_2days==1 & m68_1>96 ~ 9 ,
               bidx_01!=1 | age>=24 ~ 99 )) %>%
...
#//PNC timing for newborn
 	  IRdata <- IRdata %>%
 	    mutate(rh_pnc_nb_timing =
 	             case_when(
 	               (m71_1 >=207 & m71_1<=297) | (m71_1>=301 & m71_1<397) ~ 0 ,
 	               m70_1==0 | m70_1==9 ~ 0,
 	               (m72_1>29 & m72_1<97) | m72_1==NA ~ 0,
 	               m71_1 ==100 ~ 1 ,
 	               ...
 	               m71_1  %in% c(198, 199, 298, 299, 298, 399, 998, 999)  ~ 9 ,
 	               bidx_01!=1 | age>=24 ~ 99 )) %>%
...
#//PNC within 2days for newborn	
 	  IRdata <- IRdata %>%
 	    ...
 	    set_value_labels(rh_pnc_nb_2days = c("No Visit w/in 2 days" = 0, "visit w/in 2 days" = 1 )) %>%
 	    set_variable_labels(rh_pnc_nb_2days = "PNC check within two days for newborn")
 	  
# //PNC provider for newborn
# This is country specific, please check table in final report IRdata <- IRdata %>%
 	  IRdata <- IRdata %>%
 	    mutate(rh_pnc_nb_pv =
 	           case_when(
 	             rh_pnc_nb_timing>0 & m72_1==0 ~ 0,
 	             rh_pnc_nb_2days==0 ~ 0,
 	             rh_pnc_nb_timing>0 & m72_1 ==11 ~ 1,
 	             rh_pnc_nb_timing>0 & m72_1 %in% c(12,13) ~ 2,
 	             rh_pnc_nb_timing>0 & m72_1 %in% c(14,15) ~ 3,
 	             rh_pnc_nb_timing>0 & m72_1>=16 & m52_1<=90 ~ 4,
 	             rh_pnc_nb_timing>0 & m72_1==96 ~ 5,
 	             rh_pnc_nb_timing>0 & m72_1>96 ~ 9 ,
 	             bidx_01!=1 | age>=24 ~ 99 )) %>%
 	    replace_with_na(replace = list(rh_pnc_nb_pv = c(99))) %>%
...
# *Newborn check
  IRdata <- IRdata %>%
    mutate(nbcheck =
             case_when(m70_1==1 | m70_1==1 ~ 1)) %>%
    mutate(pnc_nb_timing_all =
              case_when(
                nbcheck==1 & age<24 ~ 999,
                m76_1>=11 & m76_1<30 & age<24 ~ m75_1
              ))
...
# 		*Providers of PNC for home deliveries or checks after discharge
  IRdata <- IRdata %>%
    mutate(pnc_nb_pv_home =
             case_when(
               rh_pnc_nb_2days==1 & m72_1==0 ~ 0,
               rh_pnc_nb_2days==0 ~ 0
               rh_pnc_nb_2days==1 & m72_1 ==11 ~ 1,
               rh_pnc_nb_2days==1 & m72_1 %in% c(12,13) ~ 2,
               rh_pnc_nb_2days==1 & m72_1 %in% c(14,15) ~ 3,
               rh_pnc_nb_2days==1 & m72_1>=16 & m52_1<=90 ~ 4,
               rh_pnc_nb_2days==1 & m72_1==96 ~ 5,
               rh_pnc_nb_2days==1 & m72_1>96 ~ 9 ,
               bidx_01!=1 | age>=24 ~ 99 )) %>%
...
# 		*Providers of PNC for facility deliveries 
  IRdata <- IRdata %>%
    mutate(pnc_nb_pv_hf =
             case_when(
               rh_pnc_nb_2days==1 & m76_1==0 ~ 0,
               rh_pnc_nb_2days==0 ~ 0
               rh_pnc_nb_2days==1 & m76_1 ==11 ~ 1,
               rh_pnc_nb_2days==1 & m76_1 %in% c(12,13) ~ 2,
               rh_pnc_nb_2days==1 & m76_1 %in% c(14,15) ~ 3,
               rh_pnc_nb_2days==1 & m76_1>=16 & m52_1<=90 ~ 4,
               rh_pnc_nb_2days==1 & m76_1==96 ~ 5,
               rh_pnc_nb_2days==1 & m76_1>96 ~ 9 ,
               bidx_01!=1 | age>=24 ~ 99 )) %>%
