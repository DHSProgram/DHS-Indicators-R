...
for (i in seq_along(varlist)) {
  ...
  resn1 <- as.data.frame(fert(IRdata_FERT,Indicator=varlist[i]))
  resn1$period <- "0-2"
  resn2 <- as.data.frame(fert(IRdata_FERT,Indicator=varlist[i], Period = 60))
  resn2$period <- "0-4"
  
  resn3 <- as.data.frame(fert(IRdata_FERT1,Indicator=varlist[i], Period = 60 ))
  resn3$period <- "5-9"
  resn4 <- as.data.frame(fert(IRdata_FERT2,Indicator=varlist[i], Period = 60 ))
  resn4$period <- "10-14"
  resn5 <- as.data.frame(fert(IRdata_FERT3,Indicator=varlist[i], Period = 60 ))
  resn5$period <- "15-19"
  
  resc <- as.data.frame(rbind(fert(IRdata_FERT,Indicator=varlist[i], Class="v024"),
                              fert(IRdata_FERT,Indicator=varlist[i], Class="v025"),
                              fert(IRdata_FERT,Indicator=varlist[i], Class="v106"),
                              fert(IRdata_FERT,Indicator=varlist[i], Class="v190")))
  resc$period <- "0-2"
