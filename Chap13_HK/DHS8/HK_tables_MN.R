# /*****************************************************************************
# Program: 			HK_tables_MN.R - DHS8 update                                            
...
saveWorkbook(wb, here(chap, "Tables_HK_mn.xlsx"), overwrite = TRUE)

## create table of knowledge of medicines to treat HIV--------------------------
table_temp <- MRdata %>% 
  cross_rpct(
    cell_vars = list(mv013, mv502, mv025, mv106),
    col_vars = list(hk_knw_arv, hk_knw_mtct_meds, hk_knw_PrEP, total()),
    weight = wt, 
    total_label = c("Total 15-49", "Number of mmen"),
    total_statistic = c("w_rpct", "w_cases"),
    total_row_position = c("below"),
    expss_digits(digits=1)) %>% 
...
table_temp <- MRdata %>% 
  cross_rpct(
    ...
    weight = wt, 
    total_label = c("Total 15-49", "Number of men heard of PrEP"),
    total_statistic = c("w_rpct", "w_cases"),
    total_row_position = c("below"),
    expss_digits(digits=1)) %>% 
...
table_temp <- MRdata %>% 
  cross_rpct(
    ...
    cell_vars = list(mv013, mv502, mv025, mv106, mv190),
    ...
    total_row_position = c("below"),
    expss_digits(digits=1)) %>% 
  set_caption("Discriminatory attitudes toward people living with HIV among those who have heard of HIV")
...
## create table of knowledge of disclosure and stigma for HIV   ----------------
table_temp <- MRdata %>% 
  cross_rpct(
    subgroup = mv861==1,
    cell_vars = list(mv013, mv025, mv106, mv190),
...
# TABLES FOR CIRCUMCISION INDICATORS ----------------

## create table of circumcision  -----------------------------------------------
table_temp <- MRdata %>% 
  cross_rpct(
    cell_vars = list(mv013, mv025, mv106, mv190),
    col_vars = list(hk_circum, hk_circum_type, total()),
    weight = wt, 
    total_label = c("Total 15-49", "Number of men"),
    total_statistic = c("w_rpct", "w_cases"),
    total_row_position = c("below"),
...
## create table of knowledge of HIV among young people -------------------------
